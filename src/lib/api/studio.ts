...
// ═══════════════════════════════════════════════════════════════
// COLORIZE — Gera variação de cor de um produto
// ═══════════════════════════════════════════════════════════════
...
/**
 * Envia pedido de colorize ao N8N (fire-and-forget).
 * O N8N cria o generation record, debita créditos e processa.
 * Frontend faz polling via pollColorizeGeneration().
 */
export async function generateColorize(params: ColorizeParams): Promise<ColorizeResponse> {
  try {
    const response = await fetch(`${N8N_BASE_URL}/vizzu/colorize`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        user_id: params.userId,
        ...
        source_color: params.sourceColor,
    ...
    // 502/504 = webhook timeout, workflow continua no N8N
    if (response.status === 502 || response.status === 504) {
      console.warn(`[Colorize] Timeout do webhook (${response.status}) — workflow continua`);
    ...
    let data;
    try { data = JSON.parse(text); } catch { return { success: false, error: 'Resposta inválida.' }; }

    if (!response.ok) return { success: false, error: data.message || 'Erro ao gerar cor.' };

    return data;
  } catch (err) {
    console.error('[Colorize] Erro:', err);
    return { success: false, error: err instanceof Error ? err.message : 'Erro de rede.' };
  }
}

/**
 * Poll de status de uma geração colorize.
 * Consulta a tabela generations por ID.
 */
export async function pollColorizeGeneration(generationId: string): Promise<{
...
/**
 * Poll batch: consulta múltiplas gerações de uma vez.
 */
export async function pollColorizeGenerationsBatch(generationIds: string[]): Promise<Array<{
  id: string;
  status: 'pending' | 'processing' | 'completed' | 'failed';
  imageUrl?: string;
  errorMessage?: string;
  ...
  if (error || !data) return generationIds.map(id => ({ id, status: 'pending' as const }));

  return data.map((d: any) => ({
    id: d.id,
    status: d.status as 'pending' | 'processing' | 'completed' | 'failed',
    imageUrl: d.output_image_url || undefined,
