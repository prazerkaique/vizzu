...
export function GalleryPage({ galleryRefreshKey }: { galleryRefreshKey?: number }) {
  ...
  const filteredGroups = useMemo(() => {
    ...
    if (debouncedSearch) {
      ...
      groups = groups.filter(g =>
        g.groupLabel.toLowerCase().includes(term)
      );
    }
  ...
  const selectGroup = useCallback((group: GalleryGroup) => {
    setSelectedIds(prev => {
      const next = new Set(prev);
    ...
    <div className="flex-1 overflow-y-auto p-4 pb-24 md:p-6 md:pb-6">
      ...
      <div className={`rounded-xl border p-3 mb-4 ${isDark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-gray-200'}`}>
        ...
        <p className={`text-[10px] mt-2 ${textMuted}`}>
          {filteredItemCount === stats.total
            ? `${stats.total} criações`
            : `${filteredItemCount} de ${stats.total} criações`}
        </p>
      </div>
      ...
      {!isLoading && filteredGroups.length > 0 && (
        <>
          {paginatedGroups.map(group => {
            ...
            return (
              <div key={group.groupKey} className="mb-5">
                {/* ── Group header ── */}
                <div className={`flex items-center justify-between mb-2 pb-1.5 border-b ${isDark ? 'border-neutral-800' : 'border-gray-200/80'}`}>
                  <div className="flex items-center gap-2 min-w-0">
                    <div className="flex-shrink-0 px-1.5 py-0.5 rounded bg-gradient-to-r from-[#FF6B6B] to-[#FF9F43] text-white text-[8px] font-bold flex items-center gap-1">
                      <i className={`fas ${groupCfg.icon} text-[6px]`} />
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="relative aspect-square overflow-hidden">
                          ...
                          {/* Badge NOVO para itens recém-gerados (oculto durante seleção para não sobrepor checkbox) */}
                          {isNew && !isSelecting && (
                        ...
                        <div className="p-2">
                          <p className={`text-[10px] font-medium truncate ${textPrimary}`}>
                            {item.metadata?.angle
                              || (item.metadata?.view === 'front' ? 'Frente' : item.metadata?.view === 'back' ? 'Costas' : null)
          ...
          <p className={`text-center text-[10px] mt-2 ${textMuted}`}>
            {totalPages > 1
              ? `Página ${currentPage} de ${totalPages} — ${filteredItemCount} criações em ${filteredGroups.length} ${filteredGroups.length === 1 ? 'grupo' : 'grupos'}`
              : `${filteredItemCount} criações em ${filteredGroups.length} ${filteredGroups.length === 1 ? 'grupo' : 'grupos'}`}
          </p>
        </>
      )}
