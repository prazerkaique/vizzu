/**
 * Webhook handler logic — server-only file.
 * Separado de webhooks.tsx para evitar erro de code-splitting
 * do React Router v7 ("Server-only module referenced by client").
 */
...
// ─── PRODUCT CREATE via GraphQL (REST payload tem imagens incompletas) ──

async function handleProductCreateViaGraphQL(
  shop: string,
  connection: ConnectionForSync,
  shopifyGid: string,
  restPayload: any
  ...
  try {
    // Buscar produto COMPLETO via GraphQL (retorna TODAS as imagens)
    ...
    let normalized;
    if (data?.product) {
      normalized = normalizeGraphQLProduct(data.product);
      console.log(`[webhook] GraphQL OK: ${normalized.title} — ${normalized.media.length} images`);
    } else {
      // Fallback para REST se GraphQL falhar (improvável)
      console.warn(`[webhook] GraphQL returned null for ${shopifyGid}, falling back to REST payload`);
      normalized = normalizeWebhookProduct(restPayload);
    }
    ...
    await logSync(connection, "create", response.ok ? "completed" : "failed", {
      ...
      source: data?.product ? "graphql" : "rest_fallback",
    });

    console.log(`[webhook] Product created: ${normalized.title} (${normalized.media.length} images via GraphQL)`);
  } catch (e) {
    console.error("[webhook] Error creating product via GraphQL:", e);
...
async function handleProductUpdate(
  ...
  try {
    ...
    if (!maps || maps.length === 0) {
      // Produto não existe no Vizzu — importar via GraphQL (REST payload tem imagens incompletas!)
      console.log(`[webhook] Product ${shopifyGid} not in Vizzu, importing via GraphQL`);
      await handleProductCreateViaGraphQL(shop, connection, shopifyGid, payload);
      return;
    }
...
// ─── Webhook Action (exportado para webhooks.tsx) ─────────────────

export const handleWebhookAction = async ({ request }: ActionFunctionArgs) => {
